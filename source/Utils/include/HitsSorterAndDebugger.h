...
inline void printHits(const lcio::TrackerHitVec& hitVector) {
  int nHits = hitVector.size();
  for (int itHit = 0; itHit < nHits; itHit++) {
    // Get the tracker hit and print global coordinates of the hit
    lcio::TrackerHitPlane* hit = static_cast<lcio::TrackerHitPlane*>(hitVector.at(itHit));
...
inline void printHits(const lcio::Track* track) {
  const lcio::TrackerHitVec& hitVector = track->getTrackerHits();
  printHits(hitVector);
  return;
}
